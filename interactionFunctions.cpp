...
void doLook(const int x, const int y, const char lookingDirection) {
    ...
    getLookingAtLocation(x, y, lookingDirection, doLookAtX, doLookAtY);
    char mapSquare = getMapSquare(doLookAtX, doLookAtY);
    switch(mapSquare) {
...
void doTake(const int x, const int y, const char lookingDirection) {
    ...
    switch(mapSquare) {
        ...
        case MAP_SQUARE_HEALTH:
            lastMessage = "You pick up a first aid kit.";
            success = true;
            break;
    }
    if (success) {
        inventoryAdd(mapSquare);
        clearMapSquare(doLookAtX, doLookAtY, mapSquare);
    }
}
...
void doUse(const int x, const int y, const char lookingDirection) {
    ...
    if (mapSquare == MAP_SQUARE_CHASM && itemToUse == MAP_SQUARE_PLANK) {
        ...
        setMapSquare(doLookAtX, doLookAtY, mapSquare, MAP_SQUARE_PLANK_SET);
        ...
        inventoryUse(itemToUse);
        setMapSquare(doLookAtX, doLookAtY, mapSquare, MAP_SQUARE_ROPE_TIED);
        ...
        if (playerIsLookingAt(werewolfX, werewolfY) && manhattanDistance(playerX, playerY, werewolfX, werewolfY) <= SLINGSHOT_DISTANCE) {
            inventoryUse(MAP_SQUARE_PEBBLE);
            int damage = rand() % SLINGSHOT_MAX_DAMAGE + 1;
...
void doCheckForPlayerDamage() {
    if (werewolfX == playerX && werewolfY == playerY && werewolfIsAlive()) {
        int damage = rand() % WEREWOLF_MAX_DAMAGE + 1;
        doPlayerHit(damage);
        ...
        if (playerIsAlive()) {
            ...
            string verbToUse = VERBS[rand() % VERBS_COUNT];
            ...
            lastMessage += (werewolfRelocated ? " of damage and has been teleported to a random location in the maze." : " of damage. BEWARE WOLF NOT TELEPORTED!!!");
        } else {
