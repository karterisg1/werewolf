...
bool setMapSquare(const int x, const int y, const char currentValue, const char newValue) {
    bool result = false;
    if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT && getMapSquare(x, y) == currentValue) {
        MAP[y][x] = newValue;
        result = true;
    }
    return result;
...
bool canSeePast(const int x, const int y) {
    ...
    return (mapSquare == MAP_SQUARE_EMPTY     ||
            ...
            mapSquare == MAP_SQUARE_ROPE_TIED ||
            mapSquare == MAP_SQUARE_SLINGSHOT);
}
...
bool loadGame(string fileName) {
    ...
    if (!file.fail()) {
        ...
        if (mapWidth > 0 && mapHeight > 0) {
            ...
            // Consume the newline character after reading the initial values
            file.get(buffer);
...
bool saveGame(string fileName, const int playerX, const int playerY, const char lookingDirection, const char inventoryItems[], const int inventoryValues[], const int inventoryLength, const int werewolfX, const int werewolfY, const int werewolfHealth, const int werewolfStunCount) {
    bool success = false;
    ...
    if (!file.fail()) {
        ...
        for (int i = 0; i < MAP_HEIGHT; i++) {
            ...
            // No newline here.  The map in the file is packed.
        }
        file << endl;  // Add a newline after the map data
...
void loadDefaultMap() {
    ...
    FILE_INVENTORY_ITEMS[0] = '+';
    FILE_INVENTORY_VALUES[0] = 10;
...
bool getRandomEmptyLocation(const int playerX, const int playerY, const int minDistanceFromPlayer, int& emptyLocationX, int& emptyLocationY) {
    ...
    const int stopAfterThisMany = 1 + rand() % numberOfEmptyLocations;
